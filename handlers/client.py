...
from API.QIWI_API import find_tx, refill_smsvf, get_history
...
def check_service_balance(func):
    async def wrapper(query):
        data = get_balance()
        if data[0] < 300:  # data[0] == sms_balance, data[1] == admin_chat_id
...
# ___________________________________________FUNCTIONALITY______________________________________________________
# ##-----------------------------------------------------------------
# Press "Exit" button
# ##-----------------------------------------------------------------
...
async def command_region(call: CallbackQuery):
    ...
    if region_code == "fr" or region_code == "ca" or region_code == "es":  # always bad countries
...
# ##-----------------------------------------------------------------
# Get history in log via /hst command
# ##-----------------------------------------------------------------
async def get_hst(message: types.Message):
    hst = await get_history()
    for txn in hst['transactions']:
        txn_data = f"{txn.date} :: {txn.account} :: {txn.sum.amount}"
        logging.warning(txn_data)


# ##-----------------------------------------------------------------
# Update user balance via /upd userid amount
# ##-----------------------------------------------------------------
async def upd_bal(message: types.Message):
    msg_text = message.text.split()
    userid = int(msg_text[1])
    amount = int(msg_text[2])
    await update_balance(userid, amount)
...
def register_handlers_client(dp: Dispatcher):
    ...
    dp.register_message_handler(get_hst, commands=["hst"])
    dp.register_message_handler(upd_bal, commands=["upd"])
